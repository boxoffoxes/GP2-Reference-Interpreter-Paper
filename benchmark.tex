...
\vspace{.5\baselineskip}
\noindent
\emph{Recognising acyclic graphs.} The program in Figure \ref{fig:acyclicity} checks whether its input graph is acyclic. If this is the case, the program produces some output graph, otherwise it fails. Suppose we call the program \ttt{acyclic} to use it as a macro in the program \ttt{if} \ttt{acyclic} \ttt{then} $P$ \ttt{else} $Q$. Given any input graph $G$, this program will test whether $G$\/ is acyclic and, depending on the result, either execute $P$ or $Q$ on $G$.  
